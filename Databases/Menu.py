from bs4 import BeautifulSoup
import requests
from openpyexcel import load_workbook
headers = {"Accept": "*/*",
           "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36"}


class Category:
    def __init__(self, link, name):
        self.link = link
        self.name = name

    def __str__(self):
        return self.link


def get_bs(url):
    req = requests.get(url, headers)
    src = req.text
    return BeautifulSoup(src, "lxml")


def get_categories(link):
    page = get_bs(link)
    categories = []
    categories_classes = page.find_all(class_="card__link")
    for category_class in categories_classes:
        if ("promoaktsiyi" not in category_class.get('href')) and ("hit" not in category_class.get('href')):
            categories.append(Category(f"https://glovoapp.com{category_class.get('href')}", category_class.find(class_= "card__title").text.strip()))
    return categories


def save_restaurant_menu(link, wb):
    global j
    global i
    ws = wb['Menu']
    restaurant_categories = get_categories(link)
    if not restaurant_categories:
        restaurant_categories = get_bs(link).find_all(class_="list", type="LIST")
        for category in restaurant_categories:
            if "hit" not in category.get("id"):
                dishes_classes = category.find_all(class_="product-row")
                for dish in dishes_classes:
                    name = dish.find(class_="product-row__name").next.next.text.strip()
                    price = dish.find(class_="product-price__effective product-price__effective--new-card").text.strip()
                    try:
                        description = dish.find(class_="product-row__info__description").next.text.strip()
                    except AttributeError:
                        description = ""
                    ws.append([j, i, name, description, price, category.find(class_="list__title").text.strip(), ""])
                    j += 1
    else:
        for category in restaurant_categories:
            category_page = get_bs(category.link)
            dishes_classes = category_page.find_all(class_="product-row")
            for dish in dishes_classes:
                name = dish.find(class_="product-row__name").next.next.text.strip()
                price = dish.find(class_="product-price__effective product-price__effective--new-card").text.strip()
                try:
                    description = dish.find(class_="product-row__info__description").next.text.strip()
                except AttributeError:
                    description = ""
                ws.append([j, i, name, description, price, category.name, ""])
                j += 1
    return None


wb = load_workbook("Restaurants_database.xlsx")
ws = wb["Restaurants"]
restaurant_links = [ws.cell(row=i, column=15).value for i in range(1, ws.max_row + 1)][1:]
ws = wb['Menu']
ws.append(["ID", "ID_Rest", "Name", "Description", "Price", "Category"])
i = 1
j = 1
for link in restaurant_links:
    try:
        save_restaurant_menu(link, wb)
    except Exception as e:
        print(f"Menu: {e}")
        i += 1
        continue
    print(f"Rest#{i} done!")
    i += 1
    wb.save("Restaurants_database.xlsx")
wb.save("Restaurants_database.xlsx")
wb.close()